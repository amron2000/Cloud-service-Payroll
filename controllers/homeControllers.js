...
module.exports.fullEmp = function(req,res){
    var errors = [];
    if(req.query.id!=undefined)
    errors.push({msg:req.query.id});
    console.log(req.query.id);
    
    return res.render('fullEmp',{title:"admin",errors:errors});

}

module.exports.casualEmp = function(req,res){
    var errors = [];
    if(req.query.id!=undefined)
    errors.push({msg:req.query.id});
    console.log(req.query.id);
    
    return res.render('casualEmp',{title:"admin",errors:errors});
...
module.exports.employees = function(req,res){
    ...
    fullacademics.find({}, function (err, docs) {
            console.log('fullacademics');
            console.log(docs);
            var e1 =docs;
...
module.exports.delete  = function(req,res){
    ...
    fullacademics.findByIdAndDelete(id,function(err){
        if(err)
        {
...
module.exports.transactionsp = function(req,res){
    ...
    let at = {tid:parseInt(body.tid),eid:parseInt(body.eid),amount:0, month:body.month};
            ...
            console.log(at);
            fullacademics.find({ eid: at.eid}, function (err, docs) {
                if(docs.length==0)
                    {
                        console.log(err);
